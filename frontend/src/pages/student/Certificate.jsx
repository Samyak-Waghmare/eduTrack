...
const Certificate = () => {
    ...
    const instructor = courseDetails?.creator?.name || "EduLearn Instructor";
    const date = new Date().toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" });
    const certId = `EDU-${(courseId || "").slice(-6).toUpperCase()}-${(user?._id || "").slice(-6).toUpperCase()}`;

    return (
        <div className="min-h-[calc(100vh-64px)] bg-background py-10 px-4 print:bg-white print:py-0">
            {}
            <div className="max-w-[900px] mx-auto flex items-center justify-between mb-8 no-print">
            ...
            <div className="max-w-[900px] mx-auto">
                <div className="relative bg-[#ffffff] text-[#1e293b] rounded-none print:shadow-none shadow-[0_20px_60px_rgba(0,0,0,0.12)] overflow-hidden border-[12px] border-double border-[#0f172a] aspect-[1.414/1] flex flex-col justify-center print:w-[100vw] print:h-[100vh] print:border-[16px] print:m-0">
                    {}
                    ...
                    <div className="relative px-12 py-16 sm:px-24 text-center">
                        ...
                        <p className="uppercase tracking-[0.4em] text-sm text-[#64748b] font-bold mb-8">Certificate of Completion</p>
                        
                        <p className="text-[#475569] text-base font-medium italic">This is proudly presented to</p>
                        <h1 className="text-4xl sm:text-6xl font-black my-5 text-[#0f172a] tracking-tight" style={{ fontFamily: 'Georgia, serif' }}>
                            {studentName}
                        </h1>

                        <p className="text-[#475569] text-base max-w-2xl mx-auto font-medium leading-relaxed mt-6">
                            for successfully demonstrating exceptional skill and completing the online course
                        </p>
                        <h2 className="text-2xl sm:text-3xl font-bold mt-4 text-[#7c3aed] leading-tight max-w-3xl mx-auto">
                            {courseTitle}
                        </h2>

                        <div className="flex items-end justify-between mt-16 pt-8 border-t-2 border-[#e2e8f0]">
